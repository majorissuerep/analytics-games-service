...
const BUILTIN_STOCKFISH = 'builtin-stockfish-18'
...
export function ChessGame() {
  ...
  const [modelRevision, setModelRevision] = useState(BUILTIN_STOCKFISH)
  ...
  const modelOptions = useMemo(() => [
    { revisionId: BUILTIN_STOCKFISH, displayName: 'Stockfish 18', runtimeId: BUILTIN_STOCKFISH },
    { revisionId: RECKLESS_LOCAL, displayName: 'Reckless 0.9 (local)', runtimeId: RECKLESS_LOCAL },
    ...remoteModels.filter((model) => model.revisionId !== BUILTIN_STOCKFISH && model.revisionId !== RECKLESS_LOCAL),
  ], [remoteModels])
  ...
  // turn. Reckless answers through the local analyse endpoint; Stockfish WASM
  // and custom models keep their original paths. In-flight results are
  // discarded when the position moved on (epoch/fen guards) so an undo or a
  // fast human move can never let a stale engine move land.
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        } else if (modelRevision === BUILTIN_STOCKFISH) {
          engineRef.current ??= new StockfishBrowserEngine()
          move = await engineRef.current.findBestMove(localFen, levelId)
        } else {
          const legal = chess.moves({ verbose: true }).map(item => `${item.from}${item.to}${item.promotion ?? ''}`)
          const response = await fetch(`/api/chess-models/${encodeURIComponent(modelRevision)}/move`, {
            method: 'POST', headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ fen: localFen, legalMoves: legal, moveTimeMs: modelMoveTimeMs }),
          })
    ...
    return () => {
      cancelled = true
      // Always clear the busy flags, even when superseded — otherwise an undo
      // or rapid move during a search would latch them on forever.
      setEngineThinking(false)
    }
  }, [applyMove, chess, engineControl, humanColor, levelId, localFen, localResult, mode, modelMoveTimeMs, modelRevision, rethinkMs])
  ...
  function undoMove() {
    ...
    clearAnalysisState()
    const restore = undoStack[undoStack.length - required] ?? { fen: new Chess().fen(), pgn: '' }
    ...
    <main className="chess-app">
      ...
      {mode === 'setup' ? (
        <section className="chess-setup" aria-label="Chess setup">
          <div className="chess-intro"><span>♞</span><div><p className="chess-kicker">CLASSIC BOARD GAME</p><h1>Play chess your way</h1><p>Stockfish 18 and local Reckless opponents, both-sides play, pass-and-play, or a protected online room.</p></div></div>
          ...
          {setupMode === 'arena' ? <ModelArena models={modelOptions} /> : <div className="chess-setup-panel">
            ...
            {setupMode === 'bot' && <>
              ...
              {engineControl === 'auto' && <>
                ...
                {modelRevision !== BUILTIN_STOCKFISH && modelRevision !== RECKLESS_LOCAL && <div className="chess-field"><label htmlFor="model-move-time">Custom model move time</label><select id="model-move-time" value={modelMoveTimeMs} onChange={(event) => setModelMoveTimeMs(Number(event.target.value))}>{[250, 500, 1000, 2000, 3000].map((ms) => <option key={ms} value={ms}>{ms >= 1000 ? `${ms / 1000} second per move` : `${ms} ms per move`}</option>)}</select><small>How long the custom model may think on each of its turns.</small></div>}
                {modelRevision === BUILTIN_STOCKFISH && <div className="chess-field"><label htmlFor="stockfish-level">Stockfish difficulty</label><select id="stockfish-level" value={levelId} onChange={(event) => setLevelId(event.target.value as StockfishLevelId)}>{STOCKFISH_LEVELS.map((item) => <option key={item.id} value={item.id}>{item.label} · Skill {item.skill}/20</option>)}</select><small>{level.description}. Stockfish 18 WASM, {level.moveTimeMs} ms search per move.</small></div>}
              </>}
              {engineControl !== 'auto' && <>
          ...
          <footer className="chess-attribution">Rules: chess.js · Engines: Stockfish 18 WASM + Reckless 0.9 · Board: react-chessboard</footer>
        </section>
      ) : (
