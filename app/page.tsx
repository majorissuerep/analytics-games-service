...
import { SCALES, Scale } from '@/lib/scales'
import {
  getOrCreatePlayerId,
  getPlayerName,
...
// ─── Types ───────────────────────────────────────────────────────────────────
type Lang   = 'uk' | 'en'
type Phase  = 'lobby' | 'cluegiver' | 'guessing' | 'result' | 'final'

interface Player { id: string; name: string; team: number }

interface RoundData {
  turnIdx:    number
  teamIdx:    number
  cluegiver:  string         // player id
  scale:      Scale
  target:     number
  clue:       string
  guesses:    Record<string, number>  // player id → value
  locked:     Record<string, boolean> // player id → locked
  scored:     boolean
  timerStart: number | null          // Date.now() when guessing started, null = no timer
}
...
interface RoomState {
  ...
  timerSecs:    number        // 0 = disabled; 30, 60, 120, 300
  cluegivers:   Record<number, string> // teamIdx → player id, one per team, set in lobby
  usedScales:   number[]
  turnOrder:    number[]      // team indices for each turn
  turnPtr:      number
  scores:       number[]      // per-team score
  round:        RoundData | null
}

interface LeaderboardRow { id: number; name: string; score: number; rounds: number }
...
function genCode() {
  ...
  return Array.from({ length: 6 }, () => c[rand(0, c.length - 1)]).join('')
}
function scoreFor(guess: number, target: number) {
  const d = Math.abs(guess - target)
  if (d <= 1)  return { pts: 10, key: 'supermark' }
  if (d <= 4)  return { pts: 5,  key: 'bullseye' }
  if (d <= 12) return { pts: 3,  key: 'close' }
  const opposite = (guess - 50) * (target - 50) < 0
  if (opposite && d > 28) return { pts: -2, key: 'opposite' }
  return { pts: 0, key: 'far' }
}
function pickScale(used: number[]): [Scale, number, number[]] {
  let pool = [...used]
  if (pool.length >= SCALES.length) pool = []
  let idx: number
  do { idx = rand(0, SCALES.length - 1) } while (pool.includes(idx))
  return [SCALES[idx], idx, [...pool, idx]]
}
async function saveRoom(code: string, state: RoomState, hostId: string) {
  await fetch(`/api/room/${code}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ state, hostId }),
  })
}
...
export default function Game() {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [roomState?.phase, roomState?.round?.timerStart])
  ...
  async function createRoom() {
    ...
    const initialState: RoomState = {
      phase: 'lobby',
      hostId: playerId,
      players: [{ id: playerId, name: playerName, team: 0 }],
      teams: ['', ''],
      roundsPerTeam: 3,
      timerSecs: 0,
      cluegivers: {},
    ...
    await saveRoom(code, initialState, playerId)
  ...
  async function joinRoom() {
    ...
    try {
      ...
      const already = state.players.find((p: Player) => p.id === playerId)
      if (!already) {
        state.players = [...state.players, { id: playerId, name: playerName, team: -1 }]
        await saveRoom(code, state, state.hostId)
      }
      setRoomCode(code)
      codeRef.current = code
      hostRef.current = state.hostId === playerId
      ...
      setJoinError(lang === 'uk' ? 'Помилка з\'єднання' : 'Connection error')
    }
  }
  ...
  async function assignTeam(teamIdx: number) {
    ...
    const updated: RoomState = {
      ...
      players: roomState.players.map(p =>
        p.id === playerId ? { ...p, team: teamIdx } : p
      ),
    }
    setRoomState(updated)
    await saveRoom(roomCode, updated, roomState.hostId)
  }

  // ─── Lobby: volunteer as cluegiver for your team ───────────────────────────
  async function assignCluegiver(teamIdx: number, pid: string) {
    if (!roomState) return
    const updated: RoomState = {
      ...roomState,
      cluegivers: { ...(roomState.cluegivers ?? {}), [teamIdx]: pid },
    }
    setRoomState(updated)
    await saveRoom(roomCode, updated, roomState.hostId)
  }
  ...
  async function updateLobbyConfig(newTeamNames: string[], newNumTeams: number, newRounds: number, newTimer?: number) {
    ...
    const scores = Array.from({ length: newNumTeams }, () => 0)
    const teams  = newTeamNames.slice(0, newNumTeams)
    // Drop cluegiver assignments for removed teams
    const existing = roomState.cluegivers ?? {}
    const cluegivers: Record<number, string> = {}
    for (let i = 0; i < newNumTeams; i++) {
      if (existing[i]) cluegivers[i] = existing[i]
    }
    const updated: RoomState = {
      ...roomState,
      teams,
  ...
  async function startGame() {
    ...
    // Build first round
    const [scale, idx, used] = pickScale([])
    const target = rand(8, 92)
    const teamIdx = order[0]
    // Use designated cluegiver from lobby
    const cluegiver = cgs[teamIdx] ?? roomState.hostId
    ...
    const updated: RoomState = {
      ...
      turnOrder:   order,
      turnPtr:     0,
      scores:      Array.from({ length: numTeams }, () => 0),
      round,
    }
    setRoomState(updated)
    await saveRoom(roomCode, updated, roomState.hostId)
  ...
  async function submitClue() {
    ...
    const updated: RoomState = {
      ...
      round: {
        ...roomState.round,
        clue:       clueInput.trim(),
        timerStart: roomState.timerSecs > 0 ? Date.now() : null,
      },
    }
    setRoomState(updated)
    setClueInput('')
  ...
  async function lockGuess() {
    ...
    const myTeam = roomState.players.find(p => p.id === playerId)?.team ?? -1
    // Only active team guessers (not the cluegiver) may lock
    if (myTeam !== roomState.round.teamIdx) return
    if (roomState.round.cluegiver === playerId) return
    const updated: RoomState = {
      ...roomState,
      round: {
        ...roomState.round,
        guesses: { ...roomState.round.guesses, [playerId]: guess },
        locked:  { ...roomState.round.locked,  [playerId]: true },
      },
    }
    setRoomState(updated)
    await saveRoom(roomCode, updated, roomState.hostId)
  ...
  async function showResult() {
    ...
    const round  = roomState.round
    const team   = round.teamIdx
    // Average guess of team players
    const teamPlayers = roomState.players.filter(p => p.team === team)
    const guessVals   = teamPlayers
      .filter(p => p.id !== round.cluegiver && round.guesses[p.id] != null)
      .map(p => round.guesses[p.id])
    const avgGuess = guessVals.length > 0
      ? Math.round(guessVals.reduce((a, b) => a + b, 0) / guessVals.length)
      : (round.guesses[Object.keys(round.guesses)[0]] ?? 50)

    const res     = scoreFor(avgGuess, round.target)
    const scores  = roomState.scores.map((s, i) => i === team ? s + res.pts : s)
    ...
    const updated: RoomState = {
      ...
      round: {
        ...round,
        guesses: { ...round.guesses, __avg: avgGuess },
        scored: true,
      },
    }
    setRoomState(updated)
  ...
  async function nextTurn() {
    if (!roomState || !isHost) return
    const nextPtr = roomState.turnPtr + 1
    const totalTurns = roomState.roundsPerTeam * roomState.teams.length
    if (nextPtr >= totalTurns) {
      const updated: RoomState = { ...roomState, phase: 'final', turnPtr: nextPtr }
      setRoomState(updated)
      await saveRoom(roomCode, updated, roomState.hostId)
      return
    }
    const [scale, idx, used] = pickScale(roomState.usedScales)
    const teamIdx   = roomState.turnOrder[nextPtr]
    // Always use the designated cluegiver for this team
    const cluegiver = (roomState.cluegivers ?? {})[teamIdx] ?? roomState.hostId
  ...
  async function resetGame() {
    ...
    const updated: RoomState = {
      ...
      turnOrder:  [],
      turnPtr:    0,
      scores:     roomState.teams.map(() => 0),
      round:      null,
      // Keep cluegivers and team assignments from last game
    }
    setRoomState(updated)
  ...
  async function submitScore() {
    ...
    const best = Math.max(...roomState.scores)
    setSubmitting(true)
    try {
      const res = await fetch('/api/leaderboard', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          playerId,
          score: best,
          rounds: roomState.roundsPerTeam * roomState.teams.length,
        }),
      })
    ...
    return (
      <div className="wrap screen-enter">
        ...
        <div className="card">
          ...
          <button className="btn" onClick={createRoom}>{t('createRoom')}</button>
          ...
          <button className="btn ghost" onClick={joinRoom}>{t('joinRoom')}</button>
        </div>
        {showLb && <LeaderboardModal rows={lbRows} loading={lbLoading} t={t} onClose={() => setShowLb(false)} />}
        {toast && <div className="toast">{toast}</div>}
      </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {isMe && tm >= 0 && !isCgForTeam && (
                        <button
                          ...
                          onClick={() => assignCluegiver(tm, playerId)}
                    ...
                    <div key={i} className="teamchip">
                      ...
                      <input
                        ...
                        onChange={e => {
                          ...
                          setTeamNames(n)
                          updateLobbyConfig(n, numTeams, roundsPerTeam)
                        }}
                        maxLength={24}
                      />
                    </div>
  ...
  if (screen === 'game' && roomState) {
    ...
    const myTeam = rs.players.find(p => p.id === playerId)?.team ?? -1
            ...
            <div className="card">
              ...
              <div className="scalewrap">
                <div className="gauge">
                  ...
                  <Ticks />
                  <TargetBand target={round.target} />
                </div>
              </div>
    ...
    if (rs.phase === 'result' && round) {
      ...
      const res       = scoreFor(avgGuess, round.target)
      const off       = Math.abs(avgGuess - round.target)
          ...
          <div className="card">
            ...
            <div className="verdict-wrap">
              ...
              <div className="verdict-detail">
                {t('targetWas')} <b>{round.target}</b> · {t('guessWas')} <b>{avgGuess}</b> · {off} {t('off')}
              </div>
            </div>
            ...
            {isHost && (
              <button className="btn" style={{ marginTop: 16 }} onClick={isLast ? () => {
                const updated = { ...rs, phase: 'final' as Phase }
                setRoomState(updated)
                saveRoom(roomCode, updated, rs.hostId)
              } : nextTurn}>
